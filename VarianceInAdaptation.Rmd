...
Indeed there is very good evidence that no-cursor trials have higher variance than the other 3 types of reaches.

```{r}
openClosedLoopDifference()
```

Like the source variables, the percentage increase is not normaly distributed, so perhaps the median works best. We see about 28% increase in median standard deviations between closed- and open loop reaches, going from about 6.01 sd to 7.71 sd. Bayesian t-test do not require normally distributed data.

There is also evidence that the variance in reach training trials is equal to that in the two types of localization trials.

Note: this means hand localization is less noisy than no-cursor reaches, and it's spread is very comparable with the spread of regular training reaches.
...
All figures are stored in various formats.

```{r plotting_figures, include=FALSE, echo=FALSE, eval=TRUE}

for (target in c('pdf', 'svg', 'png')) {
  
  fig3_localizationProperties(target=target)
  fig4_sd_distributions(target=target)
  fig5_efferent_localization(target=target)
  fig6_variance_relations(target=target)
  fig7_variance2_learning(target=target)
  fig8_localizationSD2_implicit(target=target)
  fig9_localization_2_implicit(target=target)
