...
Here is the table with sources and numbers and age of participants in each task:

```{r}
knitr::kabel(demographicsTable())
```
...
For the training and no-cursor trials, we can calculate a bias for every target - assuming there are systematic biases depending on reach direction or location in the work space. We can then subtract those biases and determine an unbiased estimate of the standard deviation. We also subtract the biases obtained from aligned conditions and use those as a reference for the rotated conditions. So the approach for targeted reaches is to subtract the average bias for each target before determining the overall variance.

Since localization reaches are made without a target, we can not use that exact approach for localization. But we can estimate the biases throughout the work space a different way: by fitting a smoothed spline, and use that as a function of bias across different angles. For every participant we base one spline on the aligned active localization trials and one on the aligned passive localization trials. These are used as estimates for both the aligned and rotated, active and passive localization. After subtracting the bias predicted by the spline on each trial from the localization response, we can determine an unbiased estimate of standard deviation, comparably unbiased to those of the no-cursor and training trials.
...
fig0_localizationProperties()
```

There is some variance in the angular accuracy across the work space between participants, which means it will really pay off to account for this, and get an unbiased estimate of standard deviation / variance. If anything, the older participants seem less biased then the younger participants.

However, we don't do any statistics on any of this.
